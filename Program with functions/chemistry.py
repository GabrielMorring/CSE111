...
def main():
    ...
    formula = input('Enter the chemical formula for the sample: ')
    mass = input('Enter the mass in grams of the sample: ')
  
    table = make_periodic_table()

    molecule = parse_formula(formula, table)

    molar_mass = compute_molar_mass(molecule, table)
...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    #   Split the element into symbol and quantity.
        list_1 = symbol_quantity_list[i]
        element = list_1[0]
        quantity = list_1[1]

        #   Get the atomic mass for the symbol.
        table_element = periodic_table[element]
        atomic_mass = table_element[1]
