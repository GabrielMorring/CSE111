from sentence_generator import get_determiner, get_noun, get_verb, get_preposition, get_prepositional_phrase
import pytest
...
def test_get_prepostional_phrase():
    for _ in range(3):
        ...
        assert phrase[0] in ["about", "above", "across", "after", "along",
            "around", "at", "before", "behind", "below",
            "beyond", "by", "despite", "except", "for",
            "from", "in", "into", "near", "of",
            "off", "on", "onto", "out", "over",
            "past", "to", "under", "with", "without"]
        assert phrase[1] in ["the", "one"]
        assert phrase[2] in ["adult", "bird", "boy", "car", "cat",
        "child", "dog", "girl", "man", "woman"]

    for _ in range(3):
        phrase = get_prepositional_phrase(2).split()
        assert phrase[1] in ["some", "many"]
        assert phrase[2] in [ "adults", "birds", "boys", "cars", "cats",
        "children", "dogs", "girls", "men", "women"]
